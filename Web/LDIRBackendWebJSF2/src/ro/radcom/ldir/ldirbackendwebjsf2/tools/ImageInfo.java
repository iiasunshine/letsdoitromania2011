 ...
 * <li><strong>2002-01-22j</strong> Added support for file formats Amiga IFF and Sun Raster (RAS).</li>
...
/*	public static final int COLOR_TYPE_UNKNOWN = -1;
    ...
    */
    ...
    public static ImageInfo getImageInfo(String sourceName,  boolean verbose) {
        ImageInfo imageInfo = new ImageInfo();
        ImageInfo ii = new ImageInfo();
        imageInfo.setDetermineImageNumber(true);
        //boolean verbose = determineVerbosity(sourceName.isEmpty());
        InputStream in = null;
        try {
            if (sourceName.startsWith("http://")) {
                in = new URL(sourceName).openConnection().getInputStream();
            } else {
                in = new FileInputStream(sourceName);
            }
            //run(sourceName, in, imageInfo, verbose);
