...
<div id="leftColumn">
	<c:if
		test="${(role eq 'ADMIN') or (role eq 'VOLUNTEER') or (role eq 'ORGANIZER')}">
		<h:panelGroup rendered="#{teamBean.managerBool}">
			<h3>
				<a
					href="${pageContext.servletContext.contextPath}/users/echipa-add-editare.jsf">
			...
			<h3>
				<a
					href="${pageContext.servletContext.contextPath}/users/echipa-mem-editare.jsf">
			...
			<h:panelGroup rendered="#{not teamBean.equipmentBool}">
				<h3>
					<a
						href="${pageContext.servletContext.contextPath}/users/echipa-equip-detalii.jsf">
		...
		<h:panelGroup rendered="#{not teamBean.managerBool}">
			<h3>
				<a
					href="${pageContext.servletContext.contextPath}/users/echipa-del-editare.jsf">
	...
	<%--				<a4j:commandLink actionListener="#{orgBean.actionSelectTeam}"
						id="selectTeamForCleaningCommand" ajaxSingle="true">
						<f:param name="team" value="#{orgBean.teamId}" />
						<strong>Foloseste echipa pentru alocare gunoaie</strong>
					</a4j:commandLink> --%>
				</div>
			</a4j:repeat>
