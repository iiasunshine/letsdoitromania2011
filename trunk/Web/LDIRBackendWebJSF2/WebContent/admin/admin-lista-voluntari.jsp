...
<body>
	<center>
		...
		<custom:page_top_login selected="lista_utilizatori"
			role="${adminUsersManagerBean.userDetails.role}" />
		...
		<div id="pageContainer">
			<div id="contentList">
				...
				<div id="leftColumn">
					...
					<a4j:form>
						...
						<div>
							<h3>
								Anul Nasterii<br />
								<h:selectOneMenu
									value="#{adminUsersManagerBean.selectedBirthYear}">
						...
						<div>
							<h3>
								...
								<h:selectOneMenu value="#{adminUsersManagerBean.selectedRole}">
									...
									<f:selectItem itemLabel="ORGANIZER_MULTI"
										itemValue="ORGANIZER_MULTI" />
								</h:selectOneMenu>
							</h3>
						</div>
						<div>
							<h3>
								Numar mormane adaugate<br /> <label>min: </label>
								<h:inputText
									value="#{adminUsersManagerBean.selectedMinGarbages}"
									onkeypress="return numbersonly(this, event, false);"
									style="width:75px;" />
								<br /> <label>max: </label>
								<h:inputText
									value="#{adminUsersManagerBean.selectedMaxGarbages}"
									onkeypress="return numbersonly(this, event, false);"
									style="width:75px;" />
							</h3>
						</div>
						...
						<a4j:commandButton
							actionListener="#{adminUsersManagerBean.actionApplyFilter}"
							reRender="lista-utilizatori" styleClass="formButtonLeft"
					...
					<h:outputLink
						value="admin-online-users.jsf">
						<h:outputText value="Useri online" escape="false" />
					</h:outputLink>
				</div>
					...
					<a4j:outputPanel id="lista-utilizatori">
						...
						<a4j:form>
							<h1>
								...
								<br /> <br />
								<h:commandButton
									actionListener="#{adminUsersManagerBean.actionGenerateExcel}"
							...
							<a4j:repeat value="#{adminUsersManagerBean.usersList}" var="user">
								<div id="listEntryContainer">
									...
									<div class="listEntryLarge">
										<h:outputText value="#{user.recordDate}" />
									</div>
									<div class="listEntry">
										<h:outputText value="#{user.userId}" />
									</div>
									<div class="listEntryLarge">
										<h:outputText value="#{fn:length(user.garbages)}" />
									</div>
									<div class="listEntryLarge">
										<h:outputText value="#{fn:length(user.memberOf.chartedAreas)}" />
									</div>
									<div class="listEntry">
										<a4j:commandLink
											actionListener="#{adminUsersManagerBean.actionSelectUser}"
											reRender="popup_user_details" ajaxSingle="true"
											disabled="#{(adminUsersManagerBean.userDetails.role eq 'VOLUNTEER')}"
											style="#{(adminUsersManagerBean.userDetails.role eq 'VOLUNTEER') ? 'color: lightgray;' : ''}"
